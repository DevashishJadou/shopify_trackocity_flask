...
def update_metrics(metrics, row, indexes, traffic):
    ...
    metrics["Leads"] += int(row[indexes["Leads"]] or 0)
    ...
    metrics["Cart to Sales %"] = 'n/a' if metrics["ATC"] == 0 else round((metrics["Sales"] / metrics["ATC"])*100, 2)
...
def initialize_campaign_and_ad_set(data, campaign_id, row, ad_set_id):
    if campaign_id not in data:
        data[campaign_id] = {
            ...
            "CPC": 0.0, "CPA":0.0,"Cart to Sales %": 0.0,"CTR %":0.0, "CR %":0.0, "CancelOrder": 0,
            "CancelRev": 0.0, "nSales": 0, "nRevenue": 0.0, 
            "New Visits": 0, "nvisitor": 0,"ATC": 0,"visitor": 0, "nSpend": 0.0,
...
def process_ads(data, fbadsdata, row, indexes, traffic):
    ...
    data[campaign_id]["ad_sets"][ad_set_id]["ads"].append({
        ...
        "Leads": int(row[indexes["Leads"]] or 0),
        ...
        "Cart to Sales %": 'n/a' if row[indexes["ATC"]] == 0 else round((int(row[indexes["Sales"]]) / int(row[indexes["ATC"]]))*100,2),
        "CPL": 'n/a' if (row[indexes["Leads"]] or 0) == 0 else round(float(row[indexes["Spend"]]) / int(row[indexes["Leads"]]),1),
...
def get_reporttabledatafacebook():
    ...
    if user:
        ...
        adsdata = {
            ...
            "CPC": 0.0, "CPA":0.0,"Cart to Sales %": 0.0,"CTR %":0.0, "CR %":0.0, "CancelOrder": 0,
            "CancelRev": 0.0, "nSales": 0, "nRevenue": 0.0, 
            "New Visits": 0, "nvisitor":0,"ATC": 0,"visitor":0,
            "nROAS": 0.0, "nAOV": 0.0, "nCPA": 0.0, "nCPC": 0.0, "nCR %": 0.0, 
