...
for mongo_doc in result:
    ...
    urlparmas = mongo_doc['body']['urlparams']
    adsource = urlparmas.get('ad_source')
    if adsource:
        # import pdb
        # pdb.set_trace()
        adid = urlparmas.get('ad_id')
        # Insert the data into PostgreSQL
        postgres_cursor.execute(
            "INSERT INTO visitor_urlparams (sessionid, localsession, productid, adsource, adid, event_time) VALUES (%s, %s, %s, %s, %s, %s)",
            (mongo_doc['session'], mongo_doc['localsession'], str(int(mongo_doc['productid'])), adsource, adid, creation_at),
        )
